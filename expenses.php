...
<div class="row g-3 mb-4">
    <div class="col-lg-5">
        <div class="chart-card h-100">
            ...
            <div class="text-center text-muted py-4"><i class="bi bi-wallet2 fs-1 d-block mb-2 opacity-25"></i>No data yet.</div>
            <?php else: ?>
    ...
    <div class="col-lg-7">
        <div class="chart-card h-100">
            ...
            <?php if (empty($catRows)): ?>
            <div class="text-center text-muted py-4"><i class="bi bi-bar-chart fs-1 d-block mb-2 opacity-25"></i>No data yet.</div>
            <?php else: ?>
    ...
    <div class="card-body p-0">
        ...
        <div class="table-responsive">
            <table class="table mb-0" id="expTable">
                <thead>
                    <tr><th>#</th><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Added By</th><th>Actions</th></tr>
                </thead>
                ...
                <tr data-cat="<?= htmlspecialchars($e['category']) ?>" data-desc="<?= strtolower(htmlspecialchars($e['description'])) ?>">
                    ...
                    <td class="text-muted" style="font-size:.75rem;"><?= htmlspecialchars($e['creator'] ?? '—') ?></td>
                    <td>
                        <button class="btn btn-sm btn-outline-danger" onclick="deleteExpense(<?= $e['id'] ?>, '<?= htmlspecialchars(addslashes($e['description'])) ?>')">
                ...
                <tr><td colspan="7" class="text-center py-4 text-muted">No expenses recorded yet. Click "Add Expense" to start.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
...
if (CAT_DATA.length) {
    new Chart(document.getElementById('expCategoryChart'), {
        type:'doughnut',
        data:{labels:CAT_DATA.map(c=>c.category),datasets:[{data:CAT_DATA.map(c=>parseFloat(c.total)),backgroundColor:COLORS,borderWidth:2,borderColor:'#fff'}]},
        options:{cutout:'60%',plugins:{legend:{position:'bottom',labels:{font:{size:11},padding:10}},tooltip:{callbacks:{label:ctx=>' ₱'+ctx.parsed.toLocaleString('en-PH',{minimumFractionDigits:0})}}}}
    });
    new Chart(document.getElementById('expBarChart'), {
        type:'bar',
        data:{labels:CAT_DATA.map(c=>c.category),datasets:[{label:'Total Expenses (₱)',data:CAT_DATA.map(c=>parseFloat(c.total)),backgroundColor:COLORS.slice(0,CAT_DATA.length),borderRadius:8,borderSkipped:false}]},
        options:{responsive:true,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>' ₱'+ctx.parsed.y.toLocaleString('en-PH',{minimumFractionDigits:0})}}},scales:{x:{grid:{display:false}},y:{beginAtZero:true,ticks:{callback:v=>'₱'+(v>=1000?(v/1000).toFixed(0)+'k':v)}}}}
    });
}
...
document.getElementById('saveExpenseBtn').addEventListener('click', async function() {
    ...
    if (!date || !cat || !desc || !amt || amt <= 0) {
        Swal.fire({icon:'warning',title:'Incomplete',text:'Please fill in all fields correctly.'});
        return;
    }
