...
// Include payment_method — falls back to 'cash' if column doesn't exist yet
...
<div class="modal fade" id="viewModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content modal-dark" style="background:#1c2030;color:#e8ecf4;border:1px solid rgba(255,255,255,.1);">
            <div class="modal-header" style="background:linear-gradient(135deg,#e8175d,#b0134a);border:none;">
...
/* ── Export CSV ── */
function exportCSV() {
    const rows = [['ID','Date','Customer','Plate','Parts','Labor','Total','Payment']];
    document.querySelectorAll('#salesBody tr[data-date]').forEach(tr => {
        ...
        rows.push([
            ...
            cells[7].textContent.trim(),   // payment method
        ]);
    });
...
async function viewSale(id) {
    ...
    body.innerHTML = `
        <div class="row mb-3 g-2">
            <div class="col-sm-3">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:#7a8499;margin-bottom:3px;">Date</div>
                <div style="color:#e8ecf4;">${s.sale_date}</div>
            </div>
            <div class="col-sm-3">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:#7a8499;margin-bottom:3px;">Customer</div>
                <div style="color:#e8ecf4;">${s.customer_name || '—'}</div>
            </div>
            <div class="col-sm-3">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:#7a8499;margin-bottom:3px;">Plate</div>
                <div style="color:#e8ecf4;">${s.plate_number || '—'}</div>
            </div>
            <div class="col-sm-3">
                <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;color:#7a8499;margin-bottom:3px;">Payment</div>
        ...
        <div class="d-flex justify-content-between align-items-center mt-3"
            style="padding-top:12px;border-top:1px solid rgba(255,255,255,.1);">
            <div>${payBadge}</div>
            <div class="d-flex gap-4">
                <span style="font-size:.88rem;">Parts: <strong style="color:#93c5fd;">₱${parseFloat(s.parts_total).toFixed(2)}</strong></span>
                <span style="font-size:.88rem;">Labor: <strong style="color:#34d399;">₱${parseFloat(s.labor_total).toFixed(2)}</strong></span>
                <span style="font-size:1.05rem;">TOTAL: <strong style="color:#e8175d;">₱${total.toFixed(2)}</strong></span>
            </div>
        </div>`;
}
