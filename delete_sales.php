...
// Catch every PHP warning/notice so it never pollutes output
set_error_handler(function($no,$str){ send(['success'=>false,'message'=>"PHP $no: $str"]); });
set_exception_handler(function($e){ send(['success'=>false,'message'=>$e->getMessage()]); });

if(empty($_SESSION['user_id'])){
    send(['success'=>false,'message'=>'Not logged in']);
}
if(($_SESSION['role']??'manager')!=='owner'){
    send(['success'=>false,'message'=>'Owner only: managers cannot delete transactions']);
}

require_once __DIR__.'/db.php';

if(!isset($conn)||$conn->connect_error){
    send(['success'=>false,'message'=>'DB connection failed']);
}

function del($conn,$id){
    $id=(int)$id;
    if($id<=0) return false;
    // Remove child rows first so FK never blocks
