    ...
    function runForecast() {
        ...
        fetch('backend/forecast.php')
            ...
            .catch(function(e) {
                ...
                if (loadingEl) {
                    loadingEl.style.display = 'block';
                    loadingEl.innerHTML = '<p style="color:#fca5a5;"><i class="bi bi-exclamation-triangle me-2"></i>'
                        + 'Forecast error: ' + e.message + '</p>'
                        + '<button onclick="document.getElementById(\'tab-forecast-btn\').click();" '
                        + 'style="margin-top:10px;padding:8px 18px;background:#16a34a;border:none;border-radius:8px;color:#fff;cursor:pointer;">'
                        + '<i class="bi bi-arrow-clockwise me-1"></i>Retry</button>';
                }
            });
    }

    // Listen on the button with both events for maximum compatibility
    var btn = document.getElementById('tab-forecast-btn');
    if (btn) {
        btn.addEventListener('shown.bs.tab', runForecast);
        btn.addEventListener('click', function() { setTimeout(runForecast, 100); });
    }
})();
</script>
</body>
