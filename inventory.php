                    ...
                    <div class="table-responsive">
                        <table class="data-table">
                            ...
                            <tbody id="categoriesBody">
                                <tr>
                                    <td colspan="3" style="text-align:center;padding:30px;color:#7a8499;">
                                        Loading categories…
                                    </td>
                                </tr>
                            </tbody>
...
<script>
// Inline forecast loader — runs after inventory.js, guaranteed no caching issues
(function () {
    'use strict';

    let _chartInstance = null; // track Chart.js instance to destroy before re-creating
    let _fLoaded = false;
    ...
    function runForecast() {
        ...
        if (loadingEl) {
            ...
            loadingEl.innerHTML = '<div class="spinner-border" style="color:#4ade80;"></div>';
        }
        ...
        fetch('backend/forecast.php')
            ...
            .then(function (text) {
                ...
                try { json = JSON.parse(text); }
                    ...
                    throw new Error('Invalid JSON from server. Raw: ' + text.substring(0, 200));
                }
                ...
                if (fBody) {
                    if (!items.length) {
                        ...
                        fBody.innerHTML = items.map(function (item) {
                            var rowClass = (item.status === 'OUT_OF_STOCK' || item.status === 'REORDER_NOW')
                ...
                // Seasonal chart — destroy existing instance before re-creating
                var chartCanvas = document.getElementById('seasonalChart');
                if (monthly.length && chartCanvas) {
                    if (_chartInstance) {
                ...
                } else if (!monthly.length && chartCanvas) {
                    chartCanvas.parentElement.innerHTML =
                        '<p style="text-align:center;color:#4b5a6e;padding:24px;font-size:.84rem;">'
                        + 'No monthly revenue data yet. Record sales to see the seasonal demand chart.</p>';
                }
            })
            .catch(function (e) {
                _fLoaded = false; // allow retry
                if (loadingEl) {
                    loadingEl.style.display = 'block';
                    loadingEl.innerHTML = '<p style="color:#fca5a5;padding:16px 0;"><i class="bi bi-exclamation-triangle me-2"></i>'
                        + 'Forecast error: ' + e.message + '</p>'
                        + '<button onclick="document.getElementById(\'tab-forecast-btn\').click();" '
                        + 'style="padding:8px 18px;background:#16a34a;border:none;border-radius:8px;color:#fff;cursor:pointer;">'
                        + '<i class="bi bi-arrow-clockwise me-1"></i>Retry</button>';
                }
            });
    }

    // Use shown.bs.tab (fires AFTER Bootstrap finishes the tab transition — reliable)
    var btn = document.getElementById('tab-forecast-btn');
    if (btn) {
        btn.addEventListener('shown.bs.tab', function () {
            runForecast();
        });
        // Fallback for direct click before Bootstrap event fires
        btn.addEventListener('click', function () {
            setTimeout(runForecast, 80);
        });
    }
})();
</script>
