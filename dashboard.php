    ...
    $top10products = [];
    $r = $conn->query("
        SELECT si.description, COALESCE(p.code,'') AS code,
    ...
    <body>
        ...
        <script>
            ...
            function buildProducts(mode) {
                ...
                const labels = sorted.map(p => p.description.length > 22 ? p.description.substring(0, 22) + '…' : p.description);
                ...
                productsChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        scales: {
                            x: { grid: GRID, ticks: { ...TICK, callback: v => mode === 'qty' ? v : (v >= 1000 ? '₱' + (v / 1000).toFixed(0) + 'k' : '₱' + v) } },
            ...
            (function () {
                ...
                const labels = STOCKS.map(s => s.description.length > 20 ? s.description.substring(0, 20) + '…' : s.description);
