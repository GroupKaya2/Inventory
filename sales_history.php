...
.page-header {
    ...
    color: #fff; padding: 24px 28px; border-radius: 14px; margin-bottom: 24px;
}
.card { border: none; border-radius: 14px; box-shadow: 0 4px 18px rgba(0,0,0,.08); }
.table thead th {
    background: linear-gradient(135deg, #667eea, #764ba2);
    color: #fff; font-size: .78rem; font-weight: 600; border: none; padding: 10px 12px;
}
...
<div class="page-header">
    ...
    <p class="mb-0 mt-1 opacity-75">Complete record of all transactions</p>
</div>

<!-- SUMMARY PILLS -->
<div class="d-flex flex-wrap gap-2 mb-3">
    <span class="summary-pill">Total Sales: <span class="val"><?= $stats['total_sales'] ?></span></span>
...
<div class="filter-bar d-flex flex-wrap gap-2 align-items-center">
    ...
    <input type="date"  id="dateFrom"    class="form-control" style="max-width:160px;" value="<?= date('Y-m-d') ?>">
    <input type="date"  id="dateTo"      class="form-control" style="max-width:160px;" value="<?= date('Y-m-d') ?>">
    ...
    <button class="btn btn-danger  btn-sm"          onclick="deleteAllTransactions()"><i class="bi bi-trash3-fill me-1"></i>Delete All</button>
</div>
                    ...
                    >
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm btn-outline-danger"
                                onclick="deleteSale(<?= $s['id'] ?>, '<?= htmlspecialchars(addslashes($s['customer_name'] ?: 'this transaction')) ?>')"
                                title="Delete">
...
function resetFilter() {
    ...
    document.querySelectorAll('#salesBody tr').forEach(function(tr) {
        tr.style.display = '';
    });
}
...
function exportCSV() {
    ...
    if (rows.length <= 1) {
        Swal.fire({ icon: 'info', title: 'No data', text: 'Nothing to export.' });
        return;
    }
    ...
    var bom  = '\uFEFF'; // UTF-8 BOM so Excel shows ₱ correctly
...
async function deleteSale(id, name) {
    var res = await Swal.fire({
        ...
        html: '<strong>' + name + '</strong><br><small class="text-muted">All sale items will also be removed. Cannot be undone.</small>',
        icon: 'warning',
    ...
    try {
        var resp = await fetch('delete_sale.php', { method: 'POST', body: fd });
...
async function deleteAllTransactions() {
    var visible = Array.from(document.querySelectorAll('#salesBody tr[data-date]'))
        .filter(function(tr) { return tr.style.display !== 'none'; });

    if (!visible.length) {
        Swal.fire({ icon: 'info', title: 'Nothing to delete', text: 'No visible transactions.' });
        return;
    }
    ...
    var res = await Swal.fire({
        ...
        preConfirm: function(val) {
            if (val !== 'DELETE') {
                Swal.showValidationMessage('You must type DELETE exactly (uppercase)');
            }
        }
    });
    if (!res.isConfirmed) return;

    // Collect IDs from data-id attribute (reliable, no text parsing)
...
async function viewSale(id) {
    ...
    try {
        ...
        if (!data.success) {
            document.getElementById('saleDetailBody').innerHTML =
                '<p class="text-danger text-center">' + (data.message || 'Failed to load.') + '</p>';
            return;
        }
        ...
        document.getElementById('saleDetailBody').innerHTML =
            '<p class="text-danger text-center">Error: ' + e.message + '</p>';
    }
}
</script>
</body>
